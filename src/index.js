...
async function handleSubscribeGet(request, env, url) {
    ...
    const tokenData = await env['NCT-SUBSCRIBER'].prepare(
        'SELECT email FROM temp_tokens WHERE token = ? AND expires > ?'
    ).bind(token, Date.now()).first();

    if (tokenData && tokenData.email === mail) {
        await env['NCT-SUBSCRIBER'].prepare('DELETE FROM temp_tokens WHERE token = ?').bind(token).run();

        const user = await env['NCT-SUBSCRIBER'].prepare(
...
async function handleSendPsCode(request, env) {
    ...
    await env['NCT-SUBSCRIBER'].prepare(
...
async function handleSubscribePost(request, env) {
    ...
    const existing = await env['NCT-SUBSCRIBER'].prepare(
...
async function handleWebhook(request, env) {
    ...
    try {
        ...
        const subscribers = await env['NCT-SUBSCRIBER'].prepare('SELECT * FROM subscribers').all();
...
async function cleanupExpiredTokens(env) {
    await env['NCT-SUBSCRIBER'].prepare('DELETE FROM temp_tokens WHERE expires < ?').bind(Date.now()).run();
}
